...
namespace SDRSharp.Plugin.WavRecorder
{
    ...
    public unsafe class SimpleWavWriter
    {
        ...
        public SimpleWavWriter(string filename, WavSampleFormat recordingFormat, ushort channels, uint sampleRate)
        {
            _filename = filename;
            _wavSampleFormat = recordingFormat;
            _format = new WavFormatHeader(recordingFormat, channels, sampleRate);
        }
        ...
        private void WritePCM8(float* data, int length)
        {
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                _outputBuffer[(i * 2)] = (byte)((re + 1.0f) * 127.5f);
                _outputBuffer[(i * 2) + 1] = (byte)((im + 1.0f) * 127.5f);
            }
        ...
        private void WritePCM16(float* data, int length)
        {
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                var re = (Int16)(*ptr++ * 32767.5f);
                var im = (Int16)(*ptr++ * 32767.5f);

                _outputBuffer[(i * 4) + 0] = (byte)(re & 0x00ff);
                _outputBuffer[(i * 4) + 1] = (byte)(re >> 8);
                _outputBuffer[(i * 4) + 2] = (byte)(im & 0x00ff);
                _outputBuffer[(i * 4) + 3] = (byte)(im >> 8);
            }
